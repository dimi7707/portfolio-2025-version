...
const BlackHorseConverter = () => {
  const [mode, setMode] = useState<'code-to-number' | 'number-to-code'>('code-to-number');
  ...
  const convertCodeToNumber = (code: string) => {
    ...
    for (let char of code) {
      if (LETTER_TO_NUMBER[char as keyof typeof LETTER_TO_NUMBER] === undefined) return null;
  ...
  const handleConvert = () => {
    ...
    if (mode === 'code-to-number') {
      if (!/^[BLACKHORSE]{1,4}$/.test(input)) {
        setError('El código solo puede contener letras válidas (B, L, A, C, K, H, O, R, S, E) y tener entre 1 y 4 caracteres.');
        return;
      }
      ...
      if (quotient <= 0 || quotient > 9999) {
        setError('El resultado de la división debe ser un número positivo de hasta 4 dígitos.');
        return;
      }
    ...
    <div className="bhc-container">
          ...
          {editRate ? (
            <>
              <input
                ...
                autoFocus
              />
              <button className="bhc-btn bhc-btn-save" onClick={handleSaveRate}>Guardar</button>
              <button className="bhc-btn bhc-btn-cancel" onClick={handleCancelRate}>Cancelar</button>
            ...
            <>
              ...
              />
              <button className="bhc-btn bhc-btn-edit" onClick={handleEditRate}>Editar</button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="bhc-main">
        <input
          className="bhc-input"
          type="text"
          placeholder={mode === 'code-to-number' ? 'Ingresa el código (B, L, A, C, K, H, O, R, S, E)' : 'Ingresa la cantidad'}
          value={input}
          onChange={handleInputChange}
          maxLength={mode === 'code-to-number' ? 4 : undefined}
        />
        <button className="bhc-btn bhc-btn-convert" onClick={handleConvert}>Convertir</button>
      </div>
      {error && <div className="bhc-error">{error}</div>}
      {result && <div className="bhc-result">{result}</div>}
      <div className="bhc-info">
        <p><b>Referencia Black Horse:</b></p>

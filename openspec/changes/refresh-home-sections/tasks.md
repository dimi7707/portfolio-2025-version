...
- [x] 3.9 Implement desktop layout: consume `groupByCategory` to render clusters (languages, frameworks, data, infra, testing) with dashed connector lines within each cluster. Found and fixed a naming collision along the way: the "Testing" cluster label matched the "Testing" node name (ambiguous for both users and tests) — renamed the cluster label to "QA & Testing".
- [x] 3.10 Implement cross-cluster connector lines for at least languages↔frameworks and data↔infra, per design.md. Realized via a shared dashed vertical "spine" that every cluster stubs into (`.clusters::before` + `.cluster::before`) rather than per-pair coordinate math — adjacent clusters along the spine read as connected, satisfying the design.md mitigation that an exact-pair implementation isn't required for a first pass.
